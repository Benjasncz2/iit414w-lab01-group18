...
| :--- | :--- | :--- |
| Logistic Regression | `C=1.0, random_state=414` | `grid, circuit_id` |
| Random Forest | `n_estimators=100, max_depth=5, random_state=414` | `grid, circuit_id, constructor_id` |

*(Note: we omitted `alt` from the original list strictly because it wasn't available in the baseline dataset.)*

## 2. Comparison Table (initial)
...
| Logistic Regression | `grid, circuit_id` | 2024 Season | 0.82 | 0.79 | Good at capturing the strong linear link between starting P1-P5 and scoring. |
| Random Forest | `grid, circuit_id, constructor_id` | 2024 Season | 0.88 | 0.81 | Better at identifying midfield "climbers" but showing slight overfitting signs. |
